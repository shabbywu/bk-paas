...
def _setup_cluster(cluster):
    # ingress-controller == 0.22.0 必须使用正则表达式
    cluster.feature_flags[ClusterFeatureFlag.INGRESS_USE_REGEX] = True
    cluster.save()


@pytest.fixture(scope="module", autouse=True)
def _setup_ingress_nginx_controller(namespace_maker, k8s_client, ingress_nginx_ns):
    ...
    if (int(k8s_version.major), int(k8s_version.minor)) >= (1, 22):
        pytest.skip("ingress-nginx-controller(0.22.0) do not support k8s >= 1.22")
